...
perlin2 x y =
  ...
  where
    ...
    grad h x' y' =
      let h2 = h .&. 3
          u  = if h2 < 2 then x' else y'
          v  = if h2 < 2 then y' else x'
          a  = if (h2 .&. 1) == 0 then u else -u
          b  = if (h2 .&. 2) == 0 then v else -v
      in a + b

p :: U.Vector Int
p = U.fromList (perm ++ perm)
  where
    perm =
